...
- 支持单张保留/排除、本组全部保留/排除，以及一键采用推荐并进入下一组。
...
1. 选择或拖入照片文件夹。
2. 应用会使用默认的 84% 相似度和 90 秒时间范围自动分析；需要调整时展开“分析设置”重新分析。
3. 在左侧逐组查看照片；“推荐”表示组内清晰度和曝光综合得分最高。
4. 点击“采用推荐并继续”可完成当前组并自动进入下一组，也可单独保留、排除或批量选择照片。
5. 设置“已筛选”文件夹，点击顶部按钮移动所有已保留照片；如需恢复，可点击返回箭头撤销。
